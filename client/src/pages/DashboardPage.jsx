...
function DashboardPage() {
  ...
  if (!dashboardData && !loading) {
    return (
      <div className="p-6 text-center">
        <p className="text-lg text-gray-600">No dashboard data available. Please add meters, slab rates, and start a billing cycle from the Settings and Billing Cycles pages.</p>
  ...
  return (
    <div className="p-4 sm:p-6 space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        {dashboardData?.currentBillingCycle?.status === 'active' && (
          <button
            ...
            className="w-full sm:w-auto flex-shrink-0 bg-red-600 hover:bg-red-700 text-white font-semibold py-2 px-4 rounded-lg shadow hover:shadow-lg transition-all duration-200"
          >
      ...
      {showCloseCycleForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60" onClick={() => setShowCloseCycleForm(false)}>
            <div className="my-6 p-6 bg-white shadow-xl rounded-lg border w-full max-w-lg mx-4" onClick={(e) => e.stopPropagation()}>
                ...
                <div key={meter.meterId} 
                  ...
                  {meter.unitsRemainingTo500 !== null && (
                      <div className="my-4">
                        <div className="flex justify-between text-xs font-medium text-slate-500 mb-1">
                            <span>Progress to 500 Unit Limit</span>
                            <span>{meter.percentageTo500}%</span>
                        </div>
                        <div className="w-full bg-gray-200 rounded-full h-4">
                            <div className="bg-gradient-to-r from-blue-500 to-sky-400 h-4 rounded-full flex items-center justify-center text-white text-xs font-mono" 
                                 style={{ width: `${meter.percentageTo500 > 100 ? 100 : meter.percentageTo500}%` }}>
                                 {meter.percentageTo500 > 15 ? `${meter.currentCycleConsumption} units` : ''}
                            </div>
                        </div>
                      </div>
                    )}
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t mt-4">
                      ...
                      {meter.unitsRemainingTo500 !== null && (
                        <>
                            <div>
                                <p className="text-xs text-slate-500">Units to 500 Limit</p>
                                <p className="text-md font-semibold text-slate-700">{meter.unitsRemainingTo500} units</p>
                            </div>
                        </>
                      )}
                  </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t">
            <div className="bg-white shadow-md rounded-lg p-6">
              ...
              <div className="space-y-2 text-sm">
                  ...
                  {/* --- MODIFIED: Added Slab Rate Info Here --- */}
